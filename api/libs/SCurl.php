...
class SelectelStorage
{
	...
	public function putArchive($archive, $path = null) {
		...
		switch ($this->format){
			case 'json':
				$headers = array_merge($this->token, array('Accept: application/json'));
				break;
			case 'xml':
				$headers = array_merge($this->token, array('Accept: application/xml'));
				break;
			default:
				$headers = array_merge($this->token, array('Accept: text/plain'));
				break;
		}
