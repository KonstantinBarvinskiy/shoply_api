...
//            return '2';
			...
			//Копируем
//			$src = 'moduleMediaFiles/' . $module . '/' . $module_id . '/' . $md5 . '.' . $ext;
//			$container->putFile($file, $src);
			...
			$db->query("SELECT MAX(`order`) FROM `mediafiles` WHERE `module` = '$module' AND `module_id` = $module_id");
			$max_order = $db->fetch();
	...
	function StarImage($id) {
		...
		$i = $db->fetch();
		$db->query("UPDATE `images` SET `main` = 'N'  WHERE `module` = '$i[module]' AND `module_id` = '$i[module_id]' AND `main` = 'Y'");
	...
	*/
	...
	function DelFile($id) {
		...
		$db = db();
		$db->query("SELECT * FROM `mediafiles` WHERE `id` = $id");
		$i = $db->fetch();
		
		if($i['src']) $container->delete($i['src']);
		$db->query("DELETE FROM `mediafiles` WHERE `id` = $id");
		
		/*if($i['main'] == 'Y') {
			$db->query("UPDATE `mediafiles` SET `main` = 'Y' WHERE `module` = '$i[module]' AND `module_id` = '$i[module_id]' LIMIT 1");
		}*/
	...
	function SortFiles($sorting) {
		...
		foreach ($sorting as $id=>$sort) {
		    $db->query("UPDATE `mediafiles` SET `order` = '$sort' WHERE `id` = $id");
		}
		$this->UpdateData();
		return true;
	}
	...
	*/
	...
	private function UpdateData() {
		$db = db();
		$db->query("SELECT * FROM `mediafiles` ORDER BY `order`");
		$data = array();
		while ($i = $db->fetch()) {
