...
class MySQL {
	...
	function __construct() {
		...
		if($GLOBALS['config']['db']['timezone']) $this->query("SET TIME_ZONE='".$GLOBALS['config']['db']['timezone']."'");
		$this->query("SET @@session.group_concat_max_len = @@global.max_allowed_packet");
		return $this->link;
	}
	...
	function row_count() {
		...
		return is_resource($this->queryResult) ? mysqli_num_rows($this->queryResult) : mysqli_affected_rows($this->link);
	}
