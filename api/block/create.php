...
if($jwt) {
    ...
    try {
        ...
        if (isset($data->callname)) {
            $block->item["callname"] = $block->callname = $data->callname;
        } else {
                        http_response_code(400);
            echo json_encode(array("message" => "No required data"), JSON_UNESCAPED_UNICODE);
            die();
        }
        if ($block->read()->rowCount() != 0) {
                        http_response_code(400);
            echo json_encode(array("message" => "Block with this callname already exist"), JSON_UNESCAPED_UNICODE);
            die();
        }
